...
import useProcessByParent from "../customHooks/useProcessByParent";
...
export default function ProcessByCenter() {
    ...
    const {processByParentInfo, updateProcessByParentInfo} = useProcessByParent(PARENT_CENTER_ID)
 ...
 const saveNewItem = async (evt:React.FormEvent<HTMLFormElement>) => {
   ...
   newProcessInfo.code = padNumbers(processByParentInfo.length.toString(), 5, "")
    ...
    <Layout>
      ...
      <div className="responsive_table_container">
        <table className="inventary__table">
            ...
            <tbody>
                {processByParentInfo.map(process=>{
                    return(
                        <tr key={process._id}>
                    <td>{process.code}</td>
